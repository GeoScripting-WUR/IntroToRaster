...
Let's load the data with `terra`. First, we will save all the files on our disk:

```{r results='hide'}
outdir <- "output"
if (!dir.exists(outdir))
  dir.create(outdir)
sits_cube_copy(tahitiMODIS, output_dir = outdir)
```
...
# If you get bogus green files, remove them
#file.remove(grep("GREEN", MODISfiles, value = TRUE))
#(MODISfiles <- list.files(outdir, full.names=TRUE))
...
It looks a bit more pixelated, but that's certainly the same Tahiti as we saw in the Landsat imagery earlier!
