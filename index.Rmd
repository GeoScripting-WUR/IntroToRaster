...
pagetitle: "Lesson 4: Intro To Raster"
output:
...
Today's lesson is about constructing a simple spatio-temporal analysis using raster data, R, and git.
...
In a previous lecture you got introduced to the overall system architecture of R programming (see figure above) and briefly saw how to read and use vector data from a file into your R environment. These vector read/write operations were made possible thanks to the *GDAL* library. The figure above provides an overview of the connections between these elements. *GDAL* stands for *Geospatial Data Abstraction Library*. You can check the project home page at [http://www.gdal.org/](http://www.gdal.org/). You will be surprised to see that a lot of the software you have used in the past to read gridded geospatial data use GDAL (i.e.: ArcGIS, QGIS, GRASS, etc). In this lesson, we will use *GDAL* indirectly via the *terra* package. However, it is also possible to call GDAL functionalities directly through the command line from a terminal, which is equivalent to calling a `system()` command directly from within R. In addition, if you are familiar with R and its string handling utilities, it may facilitate the building of the expressions that have to be passed to GDAL. (*Note*: This is also doable in *Bash* scripting, as learned in the previous lesson, and you can even combine the two.)
...
```{r}
# Using the previously generated SpatRaster bject
...
Writing a SpatRaster object to file is achieved using the `writeRaster()` function.
...
```{r, eval=FALSE}
gewata <- rast('LE71700552001036SGS00_SR_Gewata_INT1U.tif')
```
```{r, echo=FALSE}
...
Now that we have our 166 layers SpatRaster in memory, let's write it to disk using the `writeRaster()` function. Note that we adjust the layer names to the original file names (in which information on dates is written). Also note that the data range is comprised between -10000 and +10000, therefore such a file can be stored as signed 2 byte integer (INT2S).

```{r, eval=FALSE}
# Write this file at the root of the working directory
writeRaster(x=turaStack, filename='turaStack.tif', names=list.files(path='tura/'))
```
...
Let's look for instance at how the famous NDVI index would have to be calculated from the *gewata* SpatRaster object read earlier, and that contains the spectral bands of the Landsat 7 sensor. And in case you have forgotten, the NDVI formula is as follows.
...
Note that `lapp()` can also be used in that case to obtain the same result, with the same level of RAM friendliness. The advantage of `lapp()` is that the number of input SpatRasters is less limiting. As a consequence, specifying the layers does not happen in the function call but in the `()` call instead. Here, we create a *SpatRasterDataset* using `sds()`.A SpatRasterDataset contains SpatRaster objects that are sub-datasets for the same area. They all have the same extent and resolution.

```{r}
ndviLapp <- function(x, y) {
    ndvi <- (y - x) / (x + y)
    return(ndvi)
}
ndvi3 <- lapp(x=sds(gewata[[3]], gewata[[4]]), fun=ndviLapp)
```
...
```{r}
all.equal(ndvi, ndvi3, maxcell=Inf)
```

Here we see that only the name of `ndvi2` is different. This is because for the first and third method, which resulted in `ndvi` and `ndvi3`, the layer name was copied from `gewata[[4]]`, whereas this was not the case for the second approach.

In the simple case of calculating NDVI, we were easily able to produce the same result with `app()` and `lapp()`, however, it is often the case that one function is preferable to the other. As a general rule, a calculation that needs to refer to multiple individual layers separately will be easier to set up in `lapp()` than in `app()`.
...
writeRaster(x=ndviLL, filename='gewataNDVI.tif')
```
...
> **Question**: Could you also have used `ndvi` instead of `ndviLL` for this final step? Why (not)?
```
...
tahiti <- rast('LE70530722000126_sub.grd')
tahiti
...
tahiti <- rast('data/LE70530722000126_sub.grd')
plotRGB(tahiti, 3,4,5, stretch="lin")
```
...
The value replacement function takes two arguments, x and y. Similarly to what we did earlier, x corresponds to the data we want to mask, and y to the cloud mask. Again, we add both to a SpatRasterDataset.
...
tahitiCloudFree <- lapp(x = sds(tahiti6_2, fmask), fun = cloud2NA)
...
* `SpatRasterDataset`: A collection of SpatRasters.
...
* `project()`: Reproject (and resample) a raster object to a desired coordinate reference system or reference raster object.
...
* `app()`: Apply a function to every pixel independently of a single raster object (Single or multi-layer). RAM friendly and can write output directly to disk using the `filename =` argument.
* `lapp()`: Apply a function that takes values from multiple raster objects. Similar to `app()` but for multiple objects.
