...
# Raster objects manipulations
  ...
  The actual data used in geo processing projects often comes as geo-data, stored on files such as GeoTiff or other comonly used file formats. Reading data directly from these files into the R working environment (as objects belonging to one of the 3 raster objects classes), is made possible thanks to the raster package. The three main commands for reading raster objects from file are the `raster()`, `stack()`, and `brick()` functions, refering to RasterLayer, RasterStack and RasterBrick objects respectively. \\ Writing one of the three raster objects classes to file is achieved with the `writeRaster()` function.\\
  To illustrate the reading and writing of raster files, we will use the data stored in the /inst/extdata/ directory within the rasta package. The `system.file()` command is used to access these ''external'' data saved into packages.\\
...
```{r, ndvi, echo=TRUE, fig=TRUE, include=FALSE}
plot(ndvi)
```
...
## Re-projections
By the way, we still don't know where this area is. In order to investigate that, we are going to try projecting it in Google Earth. As you know Google Earth is all in Lat/Long, so we have to get our data re-projected to lat/long first. The `projectRaster()` function allows re-projection of raster objects to any projection one can think of. As the function uses the PROJ.4\footnote{PROJ.4 ([https://trac.osgeo.org/proj/]) is the reference library (external to R) that handles cartographic projections and performs projections transformations. The rgdal package is the interface between that library and R} library to perform that operation, the `crs=` argument should receive a ''proj4'' expression. ''proj4'' expressions are strings that provide the projection parameters of cartographic projections. A central place to search for projections is the spatial reference website ([http://spatialreference.org/]), from this database you will be able to query almost any reference and retrieve it in any format, including its Proj4 expression.
...
```{r, rgb, echo=TRUE, fig=TRUE, include=FALSE}
